<template>
    <div class="GoodsClassification">
        ...
        <el-table :data="DataContainers" border style="width: 100%" :show-header="header" inline class="demo-table-expand">
            <el-table-column prop="name" label="商品类别" align="center" min-width='120'></el-table-column>
            <el-table-column prop="" label="销售金额 (元)" align="center" min-width='120'>
                <template slot-scope='scope'>
                    <div>{{ (scope.row.sales * 1).toFixed(2) }}</div>
                </template>
            </el-table-column>
            <el-table-column prop="percentage" label="销售金额占比" align="center" min-width='120'>
                <!-- <template slot-scope="scope">
                    <div>{{ ( Number( scope.row.percentage * 100 ) ).toFixed(2) }} %</div>
                </template> -->
            </el-table-column>
        </el-table>
        <!--  分页器  -->
