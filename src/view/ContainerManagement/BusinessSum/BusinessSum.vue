...
<template>
    <div class="BusinessSum">
        ...
        <div class="detailsShow">
            <div>
                <p> 货柜数 </p>
                <p> {{DataContainersShow.vendorCount}} </p>
            </div>
            <div>
                <p> 应收 (元)</p>
                <p> {{(DataContainersShow.receivable * 1).toFixed(2)}} </p>
            </div>
            <div>
                <p> 实收 (元)</p>
                <p> {{(DataContainersShow.actual * 1).toFixed(2)}} </p>
            </div>
            <div>
                <p> 优惠 (元)</p>
                <p> {{(DataContainersShow.discount * 1).toFixed(2)}} </p>
            </div>
            <div>
                <p> 订单数量 </p>
                <p> {{DataContainersShow.orderCount || 0}} </p>
            </div>

            <div>
                <p> 单均消费 </p>
                <p v-if='DataContainersShow.orderCount > 0'>{{ (DataContainersShow.actual * 1 / DataContainersShow.orderCount).toFixed(2) }}</p>
                <p v-else>0</p>
            </div>
            <div>
                <p> 顾客人数 </p>
                <p> {{DataContainersShow.userCount  || 0}} </p>
            </div>
            <div>
                <p> 人均消费 </p>
                <p v-if='DataContainersShow.userCount > 0'>{{ (DataContainersShow.actual * 1 / DataContainersShow.userCount).toFixed(2) }}</p>
                <p v-else>0</p>
            </div>

            <div>
                <p> 支付宝收款 </p>
                <p> {{(DataContainersShow.alipay * 1).toFixed(2)}} </p>
            </div>
            <div>
                <p> 微信收款 </p>
                <p> {{(DataContainersShow.wxpay * 1).toFixed(2)}} </p>
            </div>
            <div>
                <p> 储值收款 </p>
                <p> {{(DataContainersShow.balance * 1).toFixed(2)}} </p>
            </div>
        </div>
        <!--  表格展示数据  -->
        <el-table :data="DataContainers" style="width: 100%" max-height="560" :show-header="header" inline>
            <el-table-column prop="vendorId" label="货柜编号" align="center" min-width='100' fixed></el-table-column>
            <el-table-column prop="address" label="摆放地址" align="center" min-width='220'></el-table-column>
            <el-table-column prop="" label="应收（元）" align="center" min-width='120'>
                <template slot-scope='scope'>
                    <div>{{ (scope.row.receivable * 1).toFixed(2) }}</div>
                </template>
            </el-table-column>
            <el-table-column prop="" label="实收（元）" align="center" min-width='120'>
                <template slot-scope='scope'>
                    <div>{{ (scope.row.actual * 1).toFixed(2) }}</div>
                </template>
            </el-table-column>
            <el-table-column prop="" label="优惠（元）" align="center" min-width='120'>
                <template slot-scope='scope'>
                    <div>{{ (scope.row.discount * 1).toFixed(2) }}</div>
                </template>
            </el-table-column>
            <el-table-column prop="orderCount" label="订单数量" align="center" min-width='120'></el-table-column>
            <el-table-column prop="" label="单均消费" align="center" min-width='120'>
                <template slot-scope='scope'>
                    <div>{{ (scope.row.aveOrder * 1).toFixed(2) }}</div>
                </template>
            </el-table-column>
            <el-table-column prop="userCount" label="顾客人数" align="center" min-width='120'></el-table-column>
            <el-table-column prop="" label="人均消费" align="center" min-width='120'>
                <template slot-scope='scope'>
                    <div>{{ (scope.row.aveUser * 1).toFixed(2) }}</div>
                </template>
            </el-table-column>
            <el-table-column prop="" label="支付宝收款" align="center" min-width='120'>
                <template slot-scope='scope'>
                    <div>{{ (scope.row.alipay * 1).toFixed(2) }}</div>
                </template>
            </el-table-column>
            <el-table-column prop="" label="微信收款" align="center" min-width='120'>
                <template slot-scope='scope'>
                    <div>{{ (scope.row.wxpay * 1).toFixed(2) }}</div>
                </template>
            </el-table-column>
            <el-table-column prop="" label="储值收款" align="center" min-width='120'>
                <template slot-scope='scope'>
    ...
    export default {
        ...
        methods: {
            ...
            search(pageNo = 1) {
                ...
                var urlShow1 = '/v2/kpis/sales/sum?';
                

                for (var i in requestData) {
                    if (requestData[i] != '' && requestData[i] != null && requestData[i] != undefined) {
                        url += i + '=' + requestData[i] + '&';
                        urlShow1 += i + '=' + requestData[i] + '&';
                    }
                }

                url = url.slice(0, url.length - 1);
                urlShow1 = urlShow1.slice(0, urlShow1.length - 1);

                this.ObtRequestData(url)
                this.ObtRequestDataShowAcc(urlShow1)
            },
            // 请求表格数据
            ObtRequestData(url) {
                // console.log(url)
            ...
            // 请求展示总数据 应收
            ObtRequestDataShowAcc(urlShow) {
                // console.log(urlShow)

                this.$http.get(urlShow).then(res => {
                    // console.log(res)
                    this.DataContainersShow = res.data.data;
                }, err => {
                    // console.log(err)
                })
            },
