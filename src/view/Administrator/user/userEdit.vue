  ...
  <div id="userDetail">
    <el-form :model="user" ref="taskForm" :rules="taskRules" label-width="100px">
      ...
      <el-form-item class="">
        ...
        <el-button v-if="this.homie === false" type="warning" @click="resetA">重置密码1</el-button>
        <el-button v-if="this.homie === true" type="warning" @click="resetB">重置密码2</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      editDistribution(formName) {
        ...
        if (this.user.identityId === null || this.user.identityId.trim() === '') {
          this.$message.error('请输入正确的身份证号!')
          return false
        }
        if (this.user.area === null || this.user.area.trim() === '') {
          this.$message.error('请输入正确的地区!')
          return false
        }
        if (!/^[0-9]*$/.test(this.user.identityId)){
          this.$message.error('请输入正确的身份证号!')
          return false
        }
        this.$refs[formName].validate((valid) => {
